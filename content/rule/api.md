...
如果使用我们提供的Token生成工具。认证的信息可以自定义，下面的代码片段是用来生成Token：

```java
Map<String, Object> authPayload = new HashMap<String, Object>();
authPayload.put("uid", "1234");
authPayload.put("isModerator", true );

TokenGenerator tokenGenerator = new TokenGenerator("<The-First-Secret>");
String token = tokenGenerator.createToken(authPayload);
```
然后你想使用auth.uid和auth.isModerator在你的规则表达式里。例如，在下面的规则里，你想在增加一个需求，博客的版主也能修改评论，只需要添加配置`auth.isModerator == true`:
...
###### 示例
<!--
如果auth.identifier小写处理后存在于/users中，具备读权限。

```json
".read": "root.child('users').child(auth.identifier.toLowerCase()).exists()"
```
-->
...
###### 示例
<!--
如果auth.identifier大写处理后存在于/users中，具备读权限。
```json
".read": "root.child('users').child(auth.identifier.toUpperCase()).exists()"
```
-->
...
非操作
###### 说明 
在规则表达式中，!常被用来查看数据是否已经被写到节点中。
下面的校验规则规定，只有在指定的节点中没有数据的时候才能写入。
```json
".write": "!data.exists()"
```
----
